...
tresult PLUGIN_API CurveController::initialize(FUnknown* context)
{
	...
	for (int32 i = 0; i < num_curve_points; ++i)
	{
		...
		parameters.addParameter(cname, nullptr, 0, (ParamValue)i / (ParamValue)num_intervals, ParameterInfo::kCanAutomate, i);
	}
	for (int32 i = 0; i < num_curved_params; ++i)
	{
		uint32_to_str16(iindex, i + 1);
		uint32_to_str16(oindex, i + 1);
		parameters.addParameter(iname, nullptr, 0, 0., ParameterInfo::kCanAutomate, num_curve_points + 2 * i);
		parameters.addParameter(oname, nullptr, 0, 0., ParameterInfo::kCanAutomate, num_curve_points + 2 * i + 1);
	}
